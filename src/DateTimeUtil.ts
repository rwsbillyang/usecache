...
dateFormat: function dateFormat(date: Date, fmt: string) {    
 ...
 * */
formatYearDateTime: (time?: number) => {
    if(!time) return ''
    const date = new Date(time)
    return DateTimeUtil.dateFormat(date, "yyyy-MM-dd hh:mm:ss")
},
...
/**
 ...
 formatDateTime: (time?: number) => {
    if(!time) return ''
    const date = new Date(time)
    return DateTimeUtil.dateFormat(date, "MM-dd hh:mm:ss")
},
...
formatDate: (time?: number) => {
    if(!time) return ''
    const date = new Date(time)
    return DateTimeUtil.dateFormat(date, "MM月dd日")
},
 ...
 * @param duration 单位秒
 */
formatDuration : (duration?: number, attachPrompt: boolean = true, prefix="阅读：") => {
    if(!duration) return ""
